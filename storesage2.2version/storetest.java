...
class StoreFullTest {
    ...
    //1. Empty Store
    @Test
    void emptyStoreHasZeroProducts() {
        assertEquals(0, storeEmpty.numberOfProducts());
        assertNull(storeEmpty.getCheapGoods());
        assertNull(storeEmpty.getAvgPrice());
        assertEquals("No products on sale", storeEmpty.getSaleGoods());
    }
    ...
    // 8. Null addition is not allowed
    @Test
    void addNullProductFails() {
        assertEquals(3, storeWithProducts.numberOfProducts());
        assertFalse(storeWithProducts.add((Product) null));
        assertEquals(3, storeWithProducts.numberOfProducts());
    }

    // 9. Modify Product Price / Status
    @Test
